...
def test_builder_selects_dcp_group_independently_from_pcp(monkeypatch):
    requested_world_sizes = []

    def get_group(expected_world_size):
        requested_world_sizes.append(expected_world_size)
        return SimpleNamespace(world_size=expected_world_size, rank_in_group=1)

    monkeypatch.setattr(indexer_backend, "get_indexer_dcp_group", get_group)
    monkeypatch.setattr(indexer_backend, "num_compute_units", lambda device: 4)
    ...
    assert requested_world_sizes == [4]
    assert builder.dcp_world_size == 4
    assert builder.dcp_rank == 1
    assert builder.use_pcp
