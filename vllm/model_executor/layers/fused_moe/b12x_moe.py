...
def _moe_workspace_token_limit() -> int:
    """Return the per-launch B12X MoE scratch capacity, or zero if unlimited."""
    return max(_env_int("B12X_MOE_WORKSPACE_TOKEN_LIMIT", 0), 0)


def _moe_workspace_tokens(num_tokens: int) -> int:
    """Bound one B12X MoE launch without changing the logical batch size."""
    ...
    def warmup_dynamic_launches(
        ...
        *,
        ...
        for tokens in sorted(set(int(count) for count in direct_token_counts)):
            ...
            # even when B12X's high-level execution plan is otherwise
