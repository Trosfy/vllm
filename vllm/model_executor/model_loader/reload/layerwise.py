...
def _online_processing_load_numel_total(layer: torch.nn.Module) -> int:
    """Return the number of checkpoint-provided elements for ``layer``.

    Some fused projections append alignment rows that are initialized by the
    model rather than loaded from the checkpoint.  Without this annotation,
    online quantization buffers every shard until end-of-model finalization.
    """
...
def _own_deferred_accelerator_tensors(bound_args: inspect.BoundArguments) -> None:
    ...
    Streaming model loaders may yield views into a staging ring that is reused
    when iteration resumes. Layerwise processing replays bound arguments only
    after every checkpoint shard for a layer arrives, so accelerator tensors
    must be cloned before they enter that deferred queue.
    """
