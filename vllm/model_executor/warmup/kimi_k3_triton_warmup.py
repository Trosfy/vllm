...
def _warm_recurrent_kda(
    ...
    # ``kernel_warmup`` runs before the production KV cache is allocated.  A
    # non-speculative K3 never reached this code (``num_speculative_tokens`` is
    # zero), but DSpark does.  Do not require a cache binding merely to compile
    # the recurrent verification variants: a single temporary state page has
    # the exact same strides seen by the kernel and is released after warmup.
