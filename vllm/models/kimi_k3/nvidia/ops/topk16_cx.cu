...
//   - padded rows emit indices = -1 (weights untouched by the reference too)
//
...
static void topk16_sigmoid(
  ...
  TORCH_CHECK(out_weights.is_cuda() && out_weights.dtype() == torch::kFloat32 &&
                  ...
                  out_weights.numel() == rows * kimi_topk16_cx::kTopK,
  ...
  TORCH_CHECK(out_indices.is_cuda() && out_indices.dtype() == torch::kInt32 &&
                  ...
                  out_indices.numel() == rows * kimi_topk16_cx::kTopK,
  ...
  kimi_topk16_cx::topk16_sigmoid_kernel<<<
      static_cast<int>(rows), kimi_topk16_cx::kThreads, 0, stream>>>(
