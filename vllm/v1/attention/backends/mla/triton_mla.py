...
def _pick_num_kv_splits(B: int, q_num_heads: int) -> int:
    """Bucket-aware num_kv_splits for the tuned MLA decode fast path."""
...
class TritonMLAMetadataBuilder(MLACommonMetadataBuilder[MLACommonMetadata]):
    ...
    def __init__(self, *args, **kwargs):
        # Keep the bumped spec-verify threshold under DCP as well. The decode
        # path below expands DCP local lengths per query, so the semantics are
        # well-defined for DCP+spec-verify.
        ...
        # They are populated via copy_ so graph replay sees stable addresses.
        max_num_tokens = (
    ...
    def _reserve_attn_logits_workspace(self) -> None:
        ...
        if not is_workspace_manager_initialized():
            return
        # Spec-verify decode tokens may be max_num_seqs * query_len.
        B = self.vllm_config.scheduler_config.max_num_seqs * max(
