...
class Qwen3DSparkSpeculator(DFlashSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # the speculators DFlash-derived "1+N" fill-in block (anchor in its own
        # slot, N mask slots predict the draft tokens; block_size == N + 1).
        block_size = getattr(self.draft_model_config.hf_config, "block_size", None)
        self.anchor_as_first = (
            block_size is None or int(block_size) <= self.num_speculative_steps
        )
        if self.anchor_as_first:
    ...
    def load_draft_model(
        ...
        return load_qwen3_dspark_model(target_model, self.vllm_config)
    ...
    def _sample_sequential(self, num_reqs: int, head_hidden: torch.Tensor) -> None:
        ...
        # Draft-space logits; the Markov bias is added in draft space and the
        # sampled id remapped to target vocab below (no-op for full-vocab drafts).
        ...
        for i in range(n_spec):
            ...
            # Remap to target vocab and feed back to the Markov head, which
            # embeds the prev token in target space (identity for full-vocab).
            target_i = self.model.map_draft_to_target(draft_i)
            self.draft_tokens[:num_reqs, i] = target_i
            prev = target_i
    ...
    def propose(
        ...
        assert aux_hidden_states is not None, (
            "Qwen3 DSpark requires auxiliary target hidden states."
        )
        ...
        if _timing:
            ...
            if self._tacc[2] % 200 == 0:
                ...
                print(
                    ...
                    f"prep(incl ctx-kv) {self._tacc[0]/self._tacc[2]:.3f} ms/step, "
                    f"draft-forward {self._tacc[1]/self._tacc[2]:.3f} ms/step",
...
def _prepare_dspark_inputs_kernel(
    ...
    if ANCHOR_AS_FIRST:
        # DeepSpec: every slot predicts the next token (slot 0 = anchor predicts
        # the first draft token), so the sampled position is query_pos + 1.
        is_sample = is_query
        sample_off = query_off
        sample_token_pos = query_pos + 1
    else:
        # Speculators 1+N fill-in: slot 0 (anchor) is not sampled; the N mask
        # slots predict the token at their own position (query_pos).
        is_sample = is_query & (query_off > 0)
        sample_off = query_off - 1
        sample_token_pos = query_pos
    sample_idx = req_idx * num_speculative_steps + sample_off
    tl.store(out_sample_indices_ptr + sample_idx, query_idx, mask=is_sample)
    tl.store(out_sample_pos_ptr + sample_idx, sample_token_pos, mask=is_sample)
...
def prepare_dspark_inputs(
    ...
    _prepare_dspark_inputs_kernel[(num_reqs, num_blocks)](
        ...
        ANCHOR_AS_FIRST=anchor_as_first,
    )
